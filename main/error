...
public class error {
    ...
    public void getLongOrNormal() throws IOException {
        error q = new error();
        LinkedHashMap<String,String> map =  q.Long_Process("C:\\UM\\Fundamental of programming\\extracted_log");
        LinkedHashMap<String,Integer> map1 = q.Occurence("C:\\UM\\Fundamental of programming\\extracted_log");
        System.out.println();
        
        //printing output
            System.out.printf("%-20s"+"\t"+"%-20s"+"%15s","Username","Type(long/normal)","Frequency");
            System.out.println("");
            System.out.println("------------------------------------------------------------");
        for(String x: map1.keySet()){
            
            System.out.printf("%-20s"+"\t"+"%-20s"+"%15s",x,map.get(x),map1.get(x));
            System.out.println();
        }
